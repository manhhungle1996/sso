package vn.com.atomi.openbanking.authen;
...
import vn.com.atomi.openbanking.authen.utils.KeycloakUtils;
...
public class KeycloakService {
    ...
    public UserRepresentation getUserByUsername(String realmName, String username){
        if(realmName.equals(keycloakProperties.getRealm())){
            return getUserByUsername(username);
        } else{
            validateEnableAdminOperation();
            List<UserRepresentation> userRepresentation =  kcAdminClient.realm(realmName).users().search(username);
            if(userRepresentation.size()==0){
                return null;
            }
            return userRepresentation.get(0);
    ...
    public AccessTokenResponse obtainAccessToken() {
        ...
        Http http = new Http(kcConfig(), new ClientIdAndSecretCredentialsProvider());
    ...
    public CibaAuthResponse makeCibaAuthRequest(String loginHint, String scope, String bindingMessage){
        CibaAuthResponse rs = null;
        RestTemplate restTemplate = new RestTemplate();
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_FORM_URLENCODED);
        MultiValueMap<String, String> map= new LinkedMultiValueMap<String, String>();
        map.add("client_id", keycloakProperties.getResource());
        map.add("client_secret", keycloakProperties.getClientKeyPassword());
        map.add("login_hint", loginHint);
        map.add("scope", scope);
        map.add("binding_message", bindingMessage);

        HttpEntity<MultiValueMap<String, String>> request = new HttpEntity<MultiValueMap<String, String>>(map, headers);

        ResponseEntity<String> response = restTemplate.postForEntity(keycloakProperties.getAuthServerUrl() + "/realms/" + keycloakProperties.getRealm() + "/protocol/openid-connect/ext/ciba/auth", request , String.class );
        if(HttpStatus.OK.equals(response.getStatusCode())) {
            JSONObject parser = new JSONObject(response.getBody());
            rs = new CibaAuthResponse();
            rs.setAuthReqId(parser.getString("auth_req_id"));
            rs.setExpiresIn(parser.getLong("expires_in"));
            rs.setInterval(parser.getLong("interval"));
        } else {
    ...
    public AccessTokenResponse getTokenFromCibaRequest(String authReqId) {
        String url = keycloakProperties.getAuthServerUrl() + "/realms/" + keycloakProperties.getRealm() + "/protocol/openid-connect/token";
        String clientId = keycloakProperties.getResource();
        String secret = keycloakProperties.getClientKeyPassword();
    ...
    public Boolean updateCibaAuthRequest(String authorization, CibaCallBackRequest cibaCallBackRequest){
        CibaAuthResponse rs = null;
        RestTemplate restTemplate = new RestTemplate();
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        headers.set("Authorization", authorization);
        HttpEntity<CibaCallBackRequest> request = new HttpEntity<>(cibaCallBackRequest, headers);

        ResponseEntity<String> response = restTemplate.postForEntity(keycloakProperties.getAuthServerUrl() + "/realms/" + keycloakProperties.getRealm() + "/protocol/openid-connect/ext/ciba/auth/callback", request , String.class );
        System.out.println("statusCode: " + response.getStatusCode());
        System.out.println("body: " + response.getBody());
